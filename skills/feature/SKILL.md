...
2. **Pick what to build** (in this priority order):
   ...
   **Hyperstition-deadline tiebreaker (applies to 2.b only).** Before finalizing the pick in step 2.b, read the `Active Targets` section of `memory/MEMORY.md` and identify any unresolved hyperstition whose deadline falls within the next **10 days**. For each unbuilt candidate idea in the batch, check whether its description directly advances such a hyperstition — e.g. a `Chinese README` candidate advances a "MiroShark PR from Chinese-locale contributor OR Chinese-language coverage by YYYY-MM-DD" target; a `MCP Tool Catalog JSON` candidate advances an "external integrator citing MiroShark by YYYY-MM-DD" target. If exactly one candidate matches, pick it **even if a different idea ranks higher on raw impact** — the 10-day deadline window is tight enough that time-sensitive wins beat evergreen wins. If multiple candidates match, fall back to highest-impact among the matched subset. If none match, proceed with the highest-impact pick unchanged. Past application — **PR #155** (2026-06-10, Chinese README): the agent picked idea #5 from the Jun-08 batch over evergreen #2/#3/#4 candidates explicitly because Jun-15 was 5 days out; that judgment call is the rule this step encodes. Log the tiebreaker decision (or its absence) in the step 11 log entry under a `**Hyperstition tiebreaker:**` line — one short sentence naming the hyperstition + deadline + days-out, or "none — no hyperstition within 10d / no candidate match" if it didn't fire.
