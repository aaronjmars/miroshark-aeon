...
- If **both** the `gh api` fetch and the WebFetch fallback fail (404, private path, network), the premise is **unverifiable** → **drop the candidate or demote it to Monitor**. Never ship a live-behavior claim you could not confirm — an unverified premise is treated exactly like a contradicted one.
