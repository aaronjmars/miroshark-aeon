...
4. **Check open PRs** to avoid suggesting ideas that are already in progress:
   ...
   **Also check `memory/topics/pre-existing-features.md`** (if it exists) — list of features verified as already shipped on the watched repo (under a different name, path, or surface). For every candidate idea, do a case-insensitive substring match against each entry's signature keywords. If a match hits, exclude the idea from the batch and append a one-line `Excluded (pre-existing): <title> — lives at <path>` note in the article's Selection Rationale section. No re-verification is needed (pre-existing entries are permanent — features don't unship). Past pattern: 8 distinct ideas across May-20 → Jun-01 batches were re-suggested after the watched repo had already shipped them (Gallery JSON, Gallery Trending, Compare API, Compare UI, RSS Feed, Per-Sim Surface Engagement, Webhook Test Ping, Simulation Search). The `feature` skill catches these via grep and pivots, but they keep eating idea slots upstream.
