...
| Date | Title | Topic |
|------|-------|-------|
| 2026-06-24 | MiroShark's Engine Reliability Has a Bus Factor of One — and It Isn't the Owner | Contributor concentration / reverse bus-factor: in the 06-17→06-24 window the ONLY external human code merger is dan-and (Daniel Andersen, Hamburg self-hoster) — #209/#194/#189/#188; aaronjmars merged 14 PRs (wait #215, cost #208, branding #206, refactor #205, dependabot #199-202) none touching engine runtime. dan-and's arc escalates plumbing→reliability→engine: #159(neo4j/same-origin)→#178(SearXNG/Firecrawl for non-websearch LLMs)→#188(timeout)→#194(report-agent i18n)→#209(thinking-model None/JSON/budget)→open #214(+197/-69, 10 files) reaching simulation_runner.py + simulation_ipc.py + wonderwall env.py/platform.py/agent.py with BEHAVIORAL fixes (platform.running() dead-loop, env.step() sync-PyTorch starving asyncio timeout, check_env_alive() PID, _ensure_env_alive() stale-RUNNING reset, monitor STOPPED-not-FAILED). simulation_runner.py June history = only #205(6cf32a8) refactor; last behavioral commits = May notifications (#93/#87) → #214 = first proposed behavioral change since May. Counter: NOT classic bus-factor (maintainer active, merges dan-and same-day, review works); #214 OPEN/unmerged (engine not unfrozen, only proposed); sim algorithm untouched by all — concentration is runtime correctness on self-hosted path aaronjmars doesn't run (he's on hosted OpenRouter default; bugs surface where strangers self-host = priority-3 "$1 first run" audience). Anchor: Ingress-Nginx CNCF best-effort retirement (2 part-time maintainers) = quiet-path failure mode. |
| 2026-06-23 | MiroShark Stopped Marketing the $1 and Started Letting You Audit It | Cost-auditability as deliberate multi-surface workstream: the "$1 to simulate" claim wired into 3 checkable surfaces in 8 days — cost.json API (#179, a52ea34, 06-16; is_estimate+pricing_basis lower bound), public embed `~$X` pill (#190, 09a60cf, 06-19), and CLI `cost` subcommand (#208, cef787b, 06-23; cmd_cost cli.py:201, `~` prefix gated on is_estimate cli.py:220, exit code 2 when no cost, --json). All three read the SAME cost.json (one source / three readers — not independent verification; all undercount identically since untracked models=$0). Engine core untouched again. Counter: 2 of 8 days = reactive firefighting (#207 mimo-v2-flash→mimo-v2.5 06-22 + #210 blank-env fallback 06-23, resolving 06-22's dead-model article). North-star priority-3/priority-zero: stranger audits "$1" from own terminal (`python cli.py cost <id>`, no SaaS/signup). Industry contrast: Langfuse/Helicone/LangSmith cost-tracking = hosted dashboards; MiroShark's lives in the OSS repo + ships caveat in payload. |
...
| wait CLI subcommand | 2026-06-24 | PR #215 — blocks until run-status hits a terminal state (completed=0/failed+stopped=1/timeout=2); kills integrators' hand-rolled poll loops, makes `wait → report`/`cost` scriptable. argparse+urllib, no new HTTP surface. repo-actions 06-22 #2. |
