...
| Skill | Date | Notes |
|-------|------|-------|
| SMTP Completion Email Notifications | 2026-05-17 | PR #87 — fourth notification channel alongside Webhook/Discord/Slack. `SMTP_HOST` + `SMTP_TO` ship `multipart/alternative` (plain text Unicode block bars + HTML inline-CSS swatches + consensus-coloured "View simulation →" CTA). Subject `[MiroShark] <Direction>: <Scenario>` for inbox-filter triage. Transport: port 465 ⇒ `SMTP_SSL`, 587 ⇒ `SMTP`+STARTTLS, 25 ⇒ plain. Auth-optional (`SMTP_USER`/`SMTP_PASSWORD` blank routes through LAN relay). Credential-leak refusal on STARTTLS failure for credentialed connections. New `email_notify.py` (~600 LoC stdlib `smtplib`+`email.mime`+`ssl`) mirrors discord_notify/slack_notify shape; 3 dispatch sites in simulation_runner.py with same `(sim_id, status)` dedup. `/api/config/notifications` envelope extended with `email_configured`. EmbedDialog gains Email chip (4th). 34 offline unit tests + extended notifications-config tests. Closes May-16 batch idea #3 — completes 4-channel quadrant (Webhook=automation / Discord=community / Slack=ops / Email=universal). Zero new deps (streak: 25 PRs). |
| Project-Lens PR Status Verification | 2026-05-16 | aeon PR #40 — `skills/project-lens/SKILL.md` gets two new guardrails: step-5 writing guidelines require `gh pr view <num> --json state,mergedAt,updatedAt` for any PR referenced by number with verb-caching, and step-6 asserts the notification's PR-status verb (`opened`/`merged`/`closed`/`draft`) must match the article body word-for-word; on doubt re-query gh and let JSON win. Fixes the 2026-05-15 bug where the project-lens notification said "merged" while the article body correctly said "opened" (MiroShark PR #83 was still open at notify time, merged ~15min later). Notifications are the highest-visibility surface this skill emits (push to Telegram/Discord/Slack), so a status hallucination there is a user-visible factual error. 3 insertions, single file. Pattern transferable to `repo-article` + `thread-formatter` if the same drift surfaces. |
| Trajectory Chart SVG | 2026-05-16 | PR #85 — `GET /api/simulation/<id>/chart.svg` renders the belief trajectory as a pure-stdlib SVG (`xml.etree.ElementTree`). Three polylines — bullish `#22c55e` / neutral `#6b7280` / bearish `#ef4444` — on a fixed `viewBox="0 0 800 400"` with 5-line y-axis grid, adaptive round-number x-axis ticks, right-aligned three-swatch legend, scenario title truncated at 80 chars + ellipsis. Reuses `trajectory_export.build_rows` so trajectory.json schema changes flow through both surfaces. Bytewise-deterministic output (cache-key friendly, same property as reproduce.json + notebook.ipynb). 5-min `Cache-Control`; publish-gated identically to trajectory.csv; 404 on empty trajectory (no blank SVG). New `chart_svg.py` (~390 LoC stdlib) + 17 offline tests in `test_unit_chart_svg.py`. `SURFACE_KEYS` frozenset gets `chart_svg` (analytics counter). EmbedDialog gets 📈 "Trajectory chart (SVG)" section: lazy-loaded preview + Download .svg + Copy URL + paste-ready `<img>` embed snippet. Closes May-14 batch idea #3. Zero new deps (streak: 23 PRs). Other May-14 ideas: #1 PR #83, #2 already at `/api/simulation/<id>/director/events`, #4 already at `/compare/:id1?/:id2?`, #5 unbuilt. |
...
## Next Priorities
- Open MiroShark PRs: #85 (Trajectory Chart SVG, opened 2026-05-16 — closes May-14 batch idea #3); #87 (SMTP Completion Email Notifications, opened 2026-05-17 — closes May-16 batch idea #3)
...
- May-14 batch progress (3/5 resolved): #1 → PR #83 2026-05-15, #2 Director Event Timeline already exists at `/api/simulation/<id>/director/events`, #3 → PR #85 2026-05-16, #4 Comparative Run View already exists at `/compare/:id1?/:id2?`, #5 Private Share Link unbuilt
