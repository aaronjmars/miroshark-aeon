...
| aeon-update enabled | 2026-08-18 | PR #135 — weekly canon framework sync from upstream aeon-agent into this instance (3-way merge, never clobbers operator config); synced through PR #169 (`bf33365`, merged 2026-09-04) via PR #161 (`3b4c5a3`, also merged 09-04) — new `cortx-reliability` skill landed (not enabled); eyebrow lock gate bumped to v0.4.2 (see Lessons Learned); 12 pending conflicts tracked in `memory/topics/aeon-update-state.json` (eslint-lint-gate bundle still held back as one coupled change) |
