...
|------|-------|----------------|
| 2026-05-31 | MiroShark Just Shipped the Surface That Reads Backwards | PR #131 opened today — 27th surface, 1st inputs-shaped: `GET /api/simulation/<id>/clone.json` returns the create-body that produced a sim. Wire-compat w/ `POST /api/simulation/create` (same fields/defaults, `polymarket_market_count` clamp [1,5], country-normalise, demographic_filters passthrough). `simulation_requirement` echoed as project-level metadata, not in create body. Pairs w/ existing `/api/simulation/compare` to close clone→modify→diff loop. +1,205 LoC stdlib, 24 offline tests, 1h cache (vs 5min analytical). Built by Aeon. 35th PR zero-deps streak. 5 quiet days on `main` since #129 merged 2026-05-29. 1,218⭐. |
